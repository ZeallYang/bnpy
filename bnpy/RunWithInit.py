...
def RunWithInit(hmodel, dict_info, initname, dataName=None, allocModelName=None, obsModelName=None, algName=None,
    ...
    if not dict_info is None:
        best_info_dict = dict_info
    else:
        best_info_dict = None
    best_loss = np.inf
    for taskid in range(starttaskid, starttaskid + nTask):
        if not hmodel is None:
            hmodel, info_dict = _run_task_initial(
                jobname, taskid, nTask, ReqArgs, KwArgs, UnkArgs,
                dataName, allocModelName, obsModelName, algName,
                doSaveToDisk, doWriteStdOut, hmodel)
        else:
...
def _run_task_initial(jobname, taskid, nTask,
                       ReqArgs, KwArgs, UnkArgs,
                       dataName, allocModelName, obsModelName, algName,
                       doSaveToDisk, doWriteStdOut, hmodel):
...
def make_initialized_model(
    ...
    hmodel = bnpy.HModel.HModel.CreateEntireModel(
